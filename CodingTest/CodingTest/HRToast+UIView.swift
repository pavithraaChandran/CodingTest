...
let HRToastActivityPositionDefault    = "top"
...
extension UIView {
    ...
    */
    ...
    class func hr_toastTitleFontName() -> String {
        ...
        if name == nil {
            let font = UIFont.init(name: "poppins", size: 12.0)// .systemFont(ofSize: 12.0)
            name = font?.fontName
    ...
    func makeToast(message msg: String) {
        makeToast(message: msg, duration: HRToastDefaultDuration, position: HRToastPositionTop as AnyObject)
    }
    ...
    func showToast(toast: UIView) {
        showToast(toast: toast, duration: HRToastDefaultDuration, position: HRToastPositionTop as AnyObject)
    }
    ...
    fileprivate func centerPointForPosition(_ position: AnyObject, toast: UIView) -> CGPoint {
        if position is String {
            ...
            if position.lowercased == HRToastPositionTop {
                ...
                return CGPoint(x: viewSize.width/2, y: viewSize.height - 50 - toastSize.height/2 - HRToastVerticalMargin)
            } else if position.lowercased == HRToastPositionTop {
